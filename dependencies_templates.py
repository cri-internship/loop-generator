...
class DependenciesTemplates:
    ...
    def flow_dependency(self, dest_array_name: str, source_array_name: str, optimize: bool, mix_in: str) -> str:
        ...
        arr_def = (arr_name, self.ld.all_arrays[arr_name])
        if mix_in == 'random':
    ...
    def gen_random_part(self, dest_array_name: str, source_array_name: str, optimize: bool,
        ...
        if optimize:
            ...
            stmt_body['source'] = [
                f'{source_array_name}{self.ld.gen_calc_for_read(random.choice(self.ld.rand_num_of_calculations), arr_def)}']
            if dest_array_name in self.ld.literal_values_destination.keys() and source_array_name in self.ld.literal_values_source.keys():
                result = ""
            else:
            ...
            stmt_body['destination'] = [dest_array_name,
                                        f'{self.ld.gen_random_stmt(self.ld.unique_arrays_write)}']
            stmt_body['source'] = [
                f'{self.ld.gen_calc_for_read(random.choice(self.ld.rand_num_of_calculations), arr_def)[1:]}',
                f'{source_array_name}{self.ld.gen_calc_for_read(random.choice(self.ld.rand_num_of_calculations), arr_def)}']
    ...
    def gen_scalar_part(self, dest_array_name: str, source_array_name: str, optimize: bool) -> str:
        ...
        if optimize:
            stmt_body['destination'] = [dest_array_name]
            stmt_body['source'] = [
                f'{source_array_name}{random.choice(self.ld.maths_operations)}{self.ld.gen_random_scalar()}']
            ...
            stmt_body['destination'] = [dest_array_name, f'{self.ld.generate_var("float ")}']
            stmt_body['source'] = [f'{self.ld.gen_random_scalar()}',
                                   f'{source_array_name}{random.choice(self.ld.maths_operations)}{self.ld.gen_random_scalar()}']
            result = self.gen_based_on_usage_flow(stmt_body)
    ...
    def gen_based_on_usage_flow(self, stmt_body):
        ...
        if self.satisfies_flow(array):
            dest_usage = self.ld.literal_values_destination[array][1]
            source_usage = self.ld.literal_values_source[array][1]
            if dest_usage <= source_usage:
                result = ""
            else:
                result = self.gen_full_stmt_flow(stmt_body)
        elif array in self.ld.literal_values_destination.keys():
    ...
    def satisfies_flow(self, array):
        return array in self.ld.literal_values_destination.keys() and array in self.ld.literal_values_source.keys()
    ...
    def gen_random_part_anti(self, dest_array_name, source_array_name, optimize, arr_def):
        ...
        if optimize:
            ...
            stmt_body['destination'] = [f'{self.ld.gen_random_stmt(self.ld.unique_arrays_write)}',
                                        dest_array_name]
            stmt_body['source'] = [
                f'{source_array_name}{self.ld.gen_calc_for_read(random.choice(self.ld.rand_num_of_calculations), arr_def)}',
                f'{self.ld.gen_calc_for_read(random.choice(self.ld.rand_num_of_calculations), arr_def)[1:]}']
    ...
    def gen_scalar_part_anti(self, dest_array_name, source_array_name, optimize):
        ...
        if optimize:
            ...
            stmt_body['destination'] = [f'{self.ld.generate_var(self.ld.type_to_init)}', dest_array_name]
            stmt_body['source'] = [
                f'{source_array_name}{random.choice(self.ld.maths_operations)}{self.ld.gen_random_scalar()}',
                f'{self.ld.gen_random_scalar()}']
            result = self.gen_based_on_usage_anti(stmt_body, source_array_name)
        return result
    ...
    def output_dependency(self, dest_array_name, source_array_name, __, mix_in):
        ...
        if mix_in == 'random':
            ...
            stmt_body['destination'] = [f'{dest_array_name}', f'{source_array_name}']
            stmt_body['source'] = [f'{self.ld.gen_random_scalar()}', f'{self.ld.gen_random_scalar()}']
            result = self.gen_based_on_usage_output(dest_array_name, stmt_body)
        return result

    def gen_based_on_usage_output(self, dest_array_name, stmt_body):
        if dest_array_name in self.ld.literal_values_destination.keys():
            dest_usage = self.ld.literal_values_destination[dest_array_name][0]
            if dest_usage >= 2:
                result = ""
            elif dest_usage == 1:
    ...
    def input_dependency(self, dest_array_name, source_array_name, __, mix_in):
        ...
        if mix_in == 'random':
            stmt_body['destination'] = [f'{self.ld.gen_random_stmt(self.ld.unique_arrays_write)}',
                                        f'{self.ld.gen_random_stmt(self.ld.unique_arrays_write)}']
            stmt_body['source'] = [
                f'{dest_array_name}{self.ld.gen_calc_for_read(random.choice(self.ld.rand_num_of_calculations), arr_def)}',
                f'{source_array_name}{self.ld.gen_calc_for_read(random.choice(self.ld.rand_num_of_calculations), arr_def)}']
            result = self.gen_based_on_usage(source_array_name, arr_def, stmt_body)
        else:
            stmt_body['destination'] = [f'{self.ld.generate_var(self.ld.type_to_init)}',
                                        f'{self.ld.generate_var(self.ld.type_to_init)}']
            stmt_body['source'] = [f'{dest_array_name}{random.choice(self.ld.maths_operations)}{self.ld.gen_random_scalar()}',
                                   f'{source_array_name}{random.choice(self.ld.maths_operations)}{self.ld.gen_random_scalar()}']
            result = self.gen_based_on_usage(source_array_name, arr_def, stmt_body)
        return result

    def gen_based_on_usage(self, source_array_name, arr_def, stmt_body):
        if source_array_name in self.ld.literal_values_source.keys():
            source_usage = self.ld.literal_values_source[source_array_name][0]
            if source_usage >= 2:
                result = ""
            elif source_usage == 1:
